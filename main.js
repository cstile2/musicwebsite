let list_of_songs = [ "audio/hyperreal.wav", "audio/Say.wav", "audio/guitar.wav" ];
let song_titles = ["New Planet", "Say", "Guitar Sound"]
let song_artists = ["Colsen", "Colsen", "IDK"]
let song_covers = ["images/red.jpg","images/basement.jpg", "images/nectar.jpg"]
let current_song = 0;
let playing = false;
let id;
...
function OnCanPlay() {
    ...
    r.style.setProperty('--colo', 'white');
    if(want_to_play) {
        PlayMusic();
    }
    want_to_play = false;
...
function StartCurrentSong() {
    ...
    r.style.setProperty('--colo', 'gray');

    want_to_play = true;
    document.getElementById("main_audio_player").load();
    UpdateMediaPlayer();
}
function PlayMusic()
{
...
for (let i = 0; i < song_covers.length; i++) {
    document.getElementById("songlist").innerHTML += `<div class="row" data-track="`+i+`">
